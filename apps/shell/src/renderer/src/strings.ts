...
export const strings = {
  zh: {
    ...
    setSecAbout: '关于',
    setSecAi: 'AI',
    setAiProvider: 'AI 服务商',
    setAiApiKey: 'API Key',
    setAiModel: '模型',
    setAiBaseUrl: '接口地址',
    setAiSave: '保存',
    setAiSaved: '已保存',
    setAiGensparkHint: 'Genspark 无需 API Key，请在“账户”标签页登录 Genspark 账号即可使用。',
    setAiCustomHint:
      '支持 OpenAI / Anthropic / Gemini / DeepSeek 官方接口，以及任意 OpenAI 兼容服务（如 Azure OpenAI、本地部署等）。模型名请使用服务商文档中给出的 ID。保存后即可在文档、表格、PDF 等模块的 AI 面板中使用。',
    setEmail: '邮箱',
    setNotLoggedIn: '未登录',
    setViewUsage: '查看用量',
    setChange: '更改',
  ...
  en: {
    ...
    setAiSaved: 'Saved',
    setAiGensparkHint:
      'Genspark does not need an API key — sign in to your Genspark account on the Account tab to use it.',
    setAiCustomHint:
      'Supports official OpenAI / Anthropic / Gemini / DeepSeek APIs and any OpenAI-compatible endpoint (e.g. Azure OpenAI, local deployments). Use the model id from your provider’s docs. After saving, the AI panel in Docs, Sheets, PDF and other modules will use this provider.',
    setEmail: 'Email',
